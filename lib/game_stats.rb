class GameStats < Stats
  ...
  def percentage_home_wins
    tally = 0
    @game_teams.find_all {|row| tally += 1 if (row.hoa == "home" && row.result == "WIN") || (row.hoa == "away" && row.result == "LOSS") }
    (tally.to_f / @game_teams.count.to_f).round(2)
  end

  def percentage_visitor_wins
    tally = 0
    @game_teams.find_all {|row| tally += 1 if (row.hoa == "away" && row.result == "WIN") || (row.hoa == "home" && row.result == "LOSS") }
    (tally.to_f / @game_teams.count.to_f).round(2)
  end
